...
export async function GET(request: Request) {
  try { await requireAdminToken(request); const result = await runtime.DB.prepare("SELECT id, type, name, slug, client_id, issuer, authorization_url, token_url, user_info_url, scopes, enabled, created_at, updated_at FROM oauth_providers ORDER BY created_at DESC").all(); return json({ providers: result.results }); }
...
export async function POST(request: Request) {
  try {
    ...
    if (type === "logto" && !issuer) return json({ error: "Logto 配置必须填写 Issuer URL" }, 400);
    ...
    await runtime.DB.batch([
      runtime.DB.prepare("INSERT OR IGNORE INTO users (id, display_name, status, is_admin, created_at, updated_at) VALUES ('usr_system_admin', 'System Admin', 'active', 0, unixepoch(), unixepoch())"),
      runtime.DB.prepare(`INSERT INTO oauth_providers (id, type, name, slug, client_id, client_secret_ciphertext, issuer, authorization_url, token_url, user_info_url, scopes, enabled, created_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'usr_system_admin', unixepoch(), unixepoch())`).bind(providerId, type, name, slug, String(input.clientId), await encrypt(String(input.clientSecret)), issuer || null, urls.authorizationUrl, urls.tokenUrl, urls.userInfoUrl, String(input.scopes || preset.scopes), input.enabled === false ? 0 : 1),
      runtime.DB.prepare("INSERT INTO audit_logs (id, actor_user_id, action, resource_type, resource_id, metadata_json, created_at) VALUES (?, NULL, 'oauth.create', 'oauth_provider', ?, ?, unixepoch())").bind(id("aud"), providerId, JSON.stringify({ type, slug })),
    ]);
...
export async function DELETE(request: Request) {
  try { await requireAdminToken(request); const input = await request.json() as { id?: string }; const result = await runtime.DB.prepare("DELETE FROM oauth_providers WHERE id = ?").bind(input.id).run(); if (!result.meta.changes) return json({ error: "配置不存在" }, 404); await runtime.DB.prepare("INSERT INTO audit_logs (id, actor_user_id, action, resource_type, resource_id, metadata_json, created_at) VALUES (?, NULL, 'oauth.delete', 'oauth_provider', ?, '{}', unixepoch())").bind(id("aud"), input.id).run(); return json({ ok: true }); }
  catch (error) { return errorResponse(error); }
}
