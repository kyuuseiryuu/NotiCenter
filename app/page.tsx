...
type Topic = { id: string; slug: string; name: string; description: string; owner_name: string; subscriber_count: number; subscribed: number; owned: number; status: string };
type Credentials = { topic: { name: string }; ingress: string };
type Endpoint = { id: string; provider: "bark" | "ntfy" | "webhook"; label: string; config: { mapping?: Record<string, string> }; verified_at: number; last_tested_at?: number; is_default: number; subscription_count: number };
...
function MappingFields({ provider }: { provider: "ntfy" | "webhook" }) {
  const defaults: Record<string, string> = provider === "ntfy" ? { body: "message", url: "click", group: "tags" } : {};
...
export default function Home() {
  ...
  const [provider, setProvider] = useState<"bark" | "ntfy" | "webhook">("bark");
  ...
  const [adapterProvider, setAdapterProvider] = useState<"bark" | "ntfy" | "webhook">("bark");
  const [linkStep, setLinkStep] = useState<"endpoint" | "code">("endpoint");
  const [linkProvider, setLinkProvider] = useState<"bark" | "ntfy" | "webhook">("bark");
  ...
  const visible = topics.filter((topic) => `${topic.name}${topic.description}${topic.owner_name}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <main className="shell">
    ...
    <section className="content" id="discover">
      ...
      {view === "discover" && <><div className="hero"><div><p className="eyebrow">DISCOVER</p><h1>订阅值得抵达的消息。</h1><p>发布统一通知入口，将一条消息安全分发到每位订阅者的 Bark、NTFY 或 Webhook。</p></div><div className="signal-card"><div className="pulse"><span></span><i></i><b></b></div><div><strong>端到端推送已就绪</strong><small>验证码登录 · 加密终端 · 幂等分发</small></div></div></div>
      <div className="section-head"><div><h2>公开主题</h2><p>{visible.length ? "发现并订阅社区维护的通知源" : "还没有匹配的主题"}</p></div><span className="soft-count">{visible.length} 个主题</span></div>
      <div className="topic-grid">{visible.map((topic, index) => <article className="topic-card" key={topic.id}><div className="card-top"><span className={`topic-icon ${["mint", "blue", "orange"][index % 3]}`}>{["↗", "◎", "⌁"][index % 3]}</span><span className={`topic-state ${topic.status}`}>{topic.status === "active" ? "运行中" : topic.status}</span></div><h3>{topic.name}</h3><p className="owner">由 <strong>{topic.owner_name}</strong> 发布 · /{topic.slug}</p><p className="description">{topic.description || "发布者暂未填写主题介绍。"}</p><div className="tags"><span>统一推送</span><span>{topic.owned ? "我发布的" : "公开"}</span></div><div className="card-footer"><span><b>{Number(topic.subscriber_count).toLocaleString()}</b> 位订阅者</span><button className={topic.subscribed ? "subscribed" : "subscribe"} onClick={() => { setSubscribeTopic(topic); setNotice(""); }}>{topic.subscribed ? "管理客户端" : "选择客户端订阅"}</button></div></article>)}</div>
    ...
    {user === null && <div className="modal-backdrop login-backdrop"><section className="login-panel"><div className="login-brand"><span className="brand-mark">N</span> NotiCenter</div><p className="eyebrow">SECURE SIGN IN</p><h2>用通知地址登录</h2><p className="login-copy">无需密码。验证码会直接发送到你的通知终端，验证后地址将加密保存。</p>{loginStep === "endpoint" ? <form onSubmit={requestCode}><label>推送系统<select value={provider} onChange={(e) => setProvider(e.target.value as typeof provider)}><option value="bark">Bark</option><option value="ntfy">NTFY</option><option value="webhook">自定义 Webhook</option></select></label><label>通知地址<input type="url" required value={endpoint} onChange={(e) => setEndpoint(e.target.value)} placeholder={provider === "bark" ? "https://api.day.app/你的密钥" : "https://…"} /></label><button className="primary-button wide" disabled={busy}>{busy ? "正在发送…" : "发送验证码"}</button></form> : <form onSubmit={verify}><button type="button" className="back-link" onClick={() => setLoginStep("endpoint")}>← 修改通知地址</button><label>6 位验证码<input className="code-input" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))} placeholder="000000" /></label><button className="primary-button wide" disabled={busy}>{busy ? "正在验证…" : "验证并登录"}</button></form>}{notice && <p className="form-notice">{notice}</p>}<small className="privacy-note">通知地址不会公开，也不会作为主题身份展示。</small></section></div>}
    ...
    {showAdapters && <div className="modal-backdrop"><section className="dialog adapter-dialog">
      ...
      <p>一个账号可以添加多个 Bark、NTFY 或 Webhook。关联已有账号后，使用其中任意客户端登录都能管理全部设备。</p>
      <div className="endpoint-list">{endpoints.map((item) => <article className="endpoint-row" key={item.id}><span className={`provider-badge ${item.provider}`}>{item.provider.toUpperCase()}</span><div><strong>{item.label}</strong><small>{item.subscription_count} 个订阅 · {item.last_tested_at ? "测试通过" : "已验证"}</small></div><button disabled={busy} onClick={() => testEndpoint(item.id)}>发送测试</button></article>)}</div>
      <div className="adapter-divider"><span>添加新客户端</span></div>
      <form onSubmit={addEndpoint}><div className="form-grid"><label>推送类型<select value={adapterProvider} onChange={(e) => setAdapterProvider(e.target.value as typeof adapterProvider)}><option value="bark">Bark</option><option value="ntfy">NTFY</option><option value="webhook">Webhook</option></select></label><label>客户端名称<input name="label" required placeholder="例如：工作手机" /></label></div><label>通知地址<input name="endpoint" type="url" required placeholder={adapterProvider === "bark" ? "https://api.day.app/设备密钥" : adapterProvider === "ntfy" ? "https://ntfy.sh/主题名称" : "https://example.com/webhook"} /></label>{adapterProvider !== "bark" && <fieldset className="mapping-fieldset"><legend>字段映射</legend><p>NotiCenter 标准字段 → 目标载荷字段名</p><MappingFields provider={adapterProvider} /></fieldset>}<button className="primary-button wide" disabled={busy}>{busy ? "正在发送测试…" : "测试连接并添加"}</button></form>
      ...
      <section className="link-account-box">
        ...
        {linkStep === "endpoint" ? <form onSubmit={requestAccountLink}><div className="form-grid"><label>目标推送类型<select value={linkProvider} onChange={(e) => setLinkProvider(e.target.value as typeof linkProvider)}><option value="bark">Bark</option><option value="ntfy">NTFY</option><option value="webhook">Webhook</option></select></label><label>已登录过的通知地址<input type="url" required value={linkEndpoint} onChange={(e) => setLinkEndpoint(e.target.value)} placeholder={linkProvider === "bark" ? "https://api.day.app/另一台设备的密钥" : "https://…"} /></label></div><button className="ghost-button wide" disabled={busy}>{busy ? "正在发送…" : "发送账号关联验证码"}</button></form> : <form onSubmit={verifyAccountLink}><button type="button" className="back-link" onClick={() => { setLinkStep("endpoint"); setLinkCode(""); setNotice(""); }}>← 修改目标通知地址</button><label>目标客户端收到的 6 位验证码<input className="code-input" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required value={linkCode} onChange={(e) => setLinkCode(e.target.value.replace(/\D/g, ""))} placeholder="000000" /></label><button className="primary-button wide" disabled={busy}>{busy ? "正在合并…" : "确认并合并两个账号"}</button></form>}
      </section>
      {notice && <p className="form-notice">{notice}</p>}
    </section></div>}
    {subscribeTopic && <div className="modal-backdrop"><section className="dialog"><button className="dialog-close" onClick={() => setSubscribeTopic(null)}>×</button><p className="eyebrow">DELIVERY TARGET</p><h2>选择接收客户端</h2><p>订阅「{subscribeTopic.name}」后，可以向任意客户端发送测试通知。</p><div className="endpoint-list selectable">{endpoints.map((item) => { const isSubscribed = subscriptions.some((subscription) => subscription.topic_id === subscribeTopic.id && subscription.endpoint_id === item.id); return <article className="endpoint-row" key={item.id}><span className={`provider-badge ${item.provider}`}>{item.provider.toUpperCase()}</span><div><strong>{item.label}</strong><small>{isSubscribed ? "已订阅 · 可以发送测试" : item.provider === "bark" ? "未订阅 · 原生格式" : "未订阅 · 使用字段映射"}</small></div><button className={isSubscribed ? "unsubscribe-button" : "select-button"} disabled={busy} onClick={() => setEndpointSubscription(item.id, !isSubscribed)}>{isSubscribed ? "取消订阅" : "订阅"}</button><button disabled={busy || !isSubscribed} title={isSubscribed ? "发送测试通知" : "请先订阅此客户端"} onClick={() => testEndpoint(item.id, subscribeTopic.name)}>测试</button></article>; })}</div>{endpoints.length === 0 && <button className="primary-button wide" onClick={() => { setSubscribeTopic(null); setShowAdapters(true); }}>先添加通知客户端</button>}{notice && <p className="form-notice">{notice}</p>}</section></div>}
    {sendTopic && <div className="modal-backdrop"><section className="dialog manual-dialog"><button className="dialog-close" onClick={() => { setSendTopic(null); setNotice(""); }}>×</button><p className="eyebrow">MANUAL PUSH</p><h2>手动发送 · {sendTopic.name}</h2><p>按 Bark 参数类型填写；空白字段不会加入请求。设备 Key 仅供映射到 NTFY/Webhook，Bark 客户端始终使用订阅者已验证的地址。</p><form onSubmit={sendManual}><ManualSendFields /><button className="primary-button wide" disabled={busy}>{busy ? "正在分发…" : "发送到所有订阅客户端"}</button></form>{notice && <p className="form-notice">{notice}</p>}</section></div>}
    <style>{`.manual-dialog{width:min(780px,100%)}.manual-fields{display:grid;gap:15px}.boolean-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.dialog .boolean-grid label{display:flex;align-items:center;gap:8px;padding:11px;border:1px solid #dfe6e2;border-radius:9px;background:#f6f8f7}.dialog .boolean-grid input{width:auto}.manage-actions .send-button{background:#245f4d;border-color:#245f4d;color:white}@media(max-width:600px){.boolean-grid{grid-template-columns:1fr}}.delivery-status.no_recipients{background:#f1f2f1;color:#77817c}.form-notice{margin:14px 0 0!important}.selectable .endpoint-row button:disabled{opacity:.42;cursor:not-allowed}.endpoint-row .unsubscribe-button{border-color:#e7c9c6;background:#fff6f5;color:#9a4d47}.copy-feedback{position:relative;transition:transform .2s ease,background .2s ease,box-shadow .2s ease}.copy-feedback:active{transform:scale(.98)}.copy-feedback.copied{background:#2f8164;border-color:#2f8164;box-shadow:0 7px 18px #2f81643d;animation:copy-pop .35s ease}@keyframes copy-pop{0%{transform:scale(.97)}60%{transform:scale(1.025)}100%{transform:scale(1)}}`}</style>
  </main>;
}
