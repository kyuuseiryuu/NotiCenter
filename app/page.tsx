...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {showAdapters && <div className="modal-backdrop"><section className="dialog adapter-dialog"><button className="dialog-close" onClick={() => setShowAdapters(false)}>×</button><p className="eyebrow">ADAPTERS</p><h2>适配器与通知客户端</h2><p>一个账号可以添加多个 Bark、NTFY 或 Webhook。NTFY 与 Webhook 可将统一消息字段映射成目标系统需要的名称。</p><div className="endpoint-list">{endpoints.map((item) => <article className="endpoint-row" key={item.id}><span className={`provider-badge ${item.provider}`}>{item.provider.toUpperCase()}</span><div><strong>{item.label}</strong><small>{item.subscription_count} 个订阅 · {item.last_tested_at ? "测试通过" : "已验证"}</small></div><button disabled={busy} onClick={() => testEndpoint(item.id)}>发送测试</button></article>)}</div><div className="adapter-divider"><span>添加新客户端</span></div><form onSubmit={addEndpoint}><div className="form-grid"><label>推送类型<select value={adapterProvider} onChange={(e) => setAdapterProvider(e.target.value as typeof adapterProvider)}><option value="bark">Bark</option><option value="ntfy">NTFY</option><option value="webhook">Webhook</option></select></label><label>客户端名称<input name="label" required placeholder="例如：工作手机" /></label></div><label>通知地址<input name="endpoint" type="url" required placeholder={adapterProvider === "bark" ? "https://api.day.app/设备密钥" : adapterProvider === "ntfy" ? "https://ntfy.sh/主题名称" : "https://example.com/webhook"} /></label>{adapterProvider !== "bark" && <fieldset className="mapping-fieldset"><legend>字段映射</legend><p>NotiCenter 标准字段 → 目标载荷字段名</p><MappingFields provider={adapterProvider} /></fieldset>}<button className="primary-button wide" disabled={busy}>{busy ? "正在发送测试…" : "测试连接并添加"}</button></form>{notice && <p className="form-notice">{notice}</p>}</section></div>}
