...
type Endpoint = { id: string; provider: "bark" | "ntfy" | "webhook"; label: string; config: { mapping?: Record<string, string> }; verified_at: number; last_tested_at?: number; is_default: number; subscription_count: number; eligible: boolean };
...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {subscribeTopic && <div className="modal-backdrop"><section className="dialog"><button className="dialog-close" onClick={() => setSubscribeTopic(null)}>×</button><p className="eyebrow">DELIVERY TARGET</p><h2>选择接收客户端</h2><p>订阅「{subscribeTopic.name}」后，可以向有效客户端发送测试通知。</p><div className="endpoint-list selectable">{endpoints.map((item) => { const isSubscribed = subscriptions.some((subscription) => subscription.topic_id === subscribeTopic.id && subscription.endpoint_id === item.id); return <article className={`endpoint-row ${item.eligible ? "" : "endpoint-paused"}`} key={item.id}><span className={`provider-badge ${item.provider}`}>{item.provider.toUpperCase()}</span><div><strong>{item.label}</strong><small>{!item.eligible ? "超出套餐额度 · 暂停投递" : isSubscribed ? "已订阅 · 可以发送测试" : item.provider === "bark" ? "未订阅 · 原生格式" : "未订阅 · 使用字段映射"}</small></div><button className={isSubscribed ? "unsubscribe-button" : "select-button"} disabled={busy || (!item.eligible && !isSubscribed)} onClick={() => setEndpointSubscription(item.id, !isSubscribed)}>{isSubscribed ? "取消订阅" : item.eligible ? "订阅" : "已暂停"}</button><button disabled={busy || !isSubscribed || !item.eligible} title={!item.eligible ? "设备已超出套餐额度" : isSubscribed ? "发送测试通知" : "请先订阅此客户端"} onClick={() => testEndpoint(item.id, subscribeTopic.name)}>测试</button></article>; })}</div>{endpoints.length === 0 && <button className="primary-button wide" onClick={() => { setSubscribeTopic(null); setShowAdapters(true); }}>先添加通知客户端</button>}{notice && <p className="form-notice">{notice}</p>}</section></div>}
