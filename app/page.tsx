...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="content" id="discover">
      ...
      <div className="topic-grid">{visible.map((topic, index) => <article className="topic-card" key={topic.id}><div className="card-top"><span className={`topic-icon ${["mint", "blue", "orange"][index % 3]}`}>{["↗", "◎", "⌁"][index % 3]}</span><span className={`topic-state ${topic.status}`}>{topic.status === "active" ? "运行中" : topic.status}</span></div><h3>{topic.name}</h3><p className="owner">由 <strong>{topic.owner_id}</strong> 发布 · /{topic.slug}</p><p className="description">{topic.description || "发布者暂未填写主题介绍。"}</p><div className="tags"><span>统一推送</span><span>{topic.owned ? "我发布的" : "公开"}</span></div><div className="card-footer"><span><b>{Number(topic.subscriber_count).toLocaleString()}</b> 位订阅者</span><button className={topic.subscribed ? "subscribed" : "subscribe"} onClick={() => { setSubscribeTopic(topic); setNotice(""); }}>{topic.subscribed ? "管理客户端" : "选择客户端订阅"}</button></div></article>)}</div>
